    ...
    try {
        ...
        if (cml["-in_debug"]) {//if read debug input, then forget about the rest
            ...
            //read until the last processed frame
                ...
                for(size_t i=0;i<Slam.getLastProcessedFrame();i++) reader->readNextFrame();
        ...
        while (!finish && reader->getNextFrame(in_depth,in_image)) {

            if (in_image.empty() || in_depth.empty())continue;
