...
namespace reslam {
struct DebugTest{
    static void removeMapPointObservation(reslam::Map &map,uint32_t point_idx,uint32_t frame_idx){
    ...
    try {
        ...
        for(auto &p:TheMap.map_points){
            ...
            for(auto of:p.getObservingFrames()){
                ...
                Proj->setInformation(Eigen::Matrix2d::Identity()* 1./ pow(TheMap.getScaleFactor() , TheMap.getMaxOctave()-kf.und_kpts[of.second].octave));
        ...
        for(auto pose:kf_poses){
            TheMap.keyframes[pose.first].pose_f2g=reslam::se3((*pose.second)(0),(*pose.second)(1),(*pose.second)(2),(*pose.second)(3),(*pose.second)(4),(*pose.second)(5));
...
//        //remove weak links
//        for(auto &p:projectionsInGraph){
//            if(p->chi2()>5.99) reslam::DebugTest::removeMapPointObservation(TheMap,p->point_id,p->frame_id);
//        }

        //finally, markers
        for(auto pose:marker_poses)
            TheMap.map_markers[pose.first].pose_g2m=reslam::se3((*pose.second)(0),(*pose.second)(1),(*pose.second)(2),(*pose.second)(3),(*pose.second)(4),(*pose.second)(5));
